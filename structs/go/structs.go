...
// Package pds defines Golang types for the Procurement Data Standard (xs:schema/@version="12012021").
package pds
...
type HeaderBasicInformationType struct {
	...
	// Selected if the procurement instrument is a contingency contract.
	ContingencyContract TrueFalseType

	// The purpose of the procurement instrument. (see list)
	DocumentPurpose DocumentPurposeType

	// Denotes whether or not the procurement instrument is an Emergency action requiring expedited processing by the selling party.
...
type MultipleLineItemsInformationType struct {
	...
	GroupVariablePricing []struct {
		...
		ItemVariableRangePricing VariableRangePricingType
	}
...
type ClauseModificationType struct {
	...
	// Contains clauses or provisions that are modified.
...
// Labor Rates used in an award modification document.
type LaborRatesModificationType struct {
...
type MultipleLineItemsInformationModificationType struct {
	...
	GroupAddresses *struct {
		// Use when the only change for an address group is to the list of line items. The addresses are unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the set of addresses. The line item list is unchanged.
		AddressChangeOnly []struct {
			// The line item list to which the addresses being modified are associated.
			LineItemList []LineItemIdentifierType

			// Records address information that has been added, removed, or modified.
			AddressModification []AddressDetailModificationType
		}

		// Use when both the addresses and the line item list of the group addresses changes.
		BothListAndAddressChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType

			// Records address information that has been added, removed, or modified.
			AddressChange []AddressDetailModificationType
		}
	}
	...
	GroupClauses *struct {
		// Use when the only change for a clause group is to the list of line items. The clauses are unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the set of clauses. The line item list is unchanged.
		ClauseChangeOnly []struct {
			// The line item list to which the clauses being modified are associated.
			LineItemList []LineItemIdentifierType

			// Records clauses that have been added, removed, or modified.
			ClauseModification ClauseModificationType
		}

		// Use when both the clauses and the line item list of the group changes.
		BothListAndClauseChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupDiscounts *struct {
		// Use when the only change for a discount group is to the list of line items. The discounts are unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the set of item discounts. The line item list is unchanged.
		ItemDiscountsChangeOnly []struct {
			// The line item list to which the discounts being modified are associated.
			LineItemList []LineItemIdentifierType

			// Records discounts that have been added, removed, or modified.
			ItemDiscountsModification DiscountDetailModificationType1
		}

		// Use when both the discounts and the line item list of the group changes.
		BothListAndItemDiscountChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupLaborRates *struct {
		// Use when the only change for a labor rates group is to the list of line items. The labor rates are unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the labor rates. The line item list is unchanged.
		LaborRatesChangeOnly []struct {
			// The line item list to which the labor rates being modified are associated.
			LineItemList []LineItemIdentifierType

			// Records labor rates that have been added, removed, or modified.
			LaborRatesModification LaborRatesModificationType
		}

		// Use when both the labor rates and the line item list of the group labor rates changes.
		BothListAndLaborRatesChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupMiscellaneousText *struct {
		// Use when the only change for a miscellaneous text group is to the list of line items. The miscellaneous text is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the miscellaneous text. The line item list is unchanged.
		MiscellaneousTextChangeOnly []struct {
			// The line item list to which the miscellaneous text being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records miscellaneous text that has been added, removed, or modified.
			MiscellaneousTextModification MiscellaneousTextModificationType
		}

		// Use when both the miscellaneous text and the line item list of the group changes.
		BothListAndMiscellaneousTextChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupOtherAmounts *struct {
		// Use when the only change for an other amounts group is to the list of line items. The other amounts are unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the other amounts. The line item list is unchanged.
		OtherAmountsChangeOnly []struct {
			// The line item list to which the other amounts being modified is associated.
			LineItemList []LineItemIdentifierType
		...
		// Use when both the other amounts and the line item list of the group changes.
		BothListAndOtherAmountsChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupReplacementPrice *struct {
		// Use when the only change for a replacement price group is to the list of line items. The replacement price is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the replacement price. The line item list is unchanged.
		ReplacementPriceChangeOnly []struct {
			// The line item list to which the replacement price being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records the replacement price that has been added, removed, or modified.
			ReplacementPriceModification ReplacementPriceModificationType
		}

		// Use when both the replacement price and the line item list of the group replacement price changes.
		BothListAndReplacementPriceChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupRequirementsDescription *struct {
		// Use when the only change for a requirements description group is to the list of line items. The requirements description is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the requirements description. The line item list is unchanged.
		RequirementsDescriptionChangeOnly []struct {
			// The line item list to which the requirements description being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records a requirements description that has been added, removed, or modified.
			RequirementsDescriptionModification XsString
		}

		// Use when both the requirements description and the line item list of the group changes.
		BothListAndRequirementsDescriptionChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupShareRatio *struct {
		// Use when the only change for a share ratio group is to the list of line items. The share ratio is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the share ratio. The line item list is unchanged.
		ShareRatioChangeOnly []struct {
			// The line item list to which the share ratio being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records the share ratio that has been added, removed, or modified.
			ItemShareRatioModification ShareRatioModificationType
		}

		// Use when both the share ratio and the line item list of the group share ratio changes.
		BothListAndShareRatioChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType

			// Records the share ratio that has been added, removed, or modified.
			ShareRatioChange ShareRatioModificationType
		}
	}

	// Changes to the special pricing that apply to a group of procurement instrument line items.
	GroupSpecialPricing *struct {
		// Use when the only change for a special pricing group is to the list of line items. The special pricing is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the special pricing. The line item list is unchanged.
		SpecialPricingChangeOnly []struct {
			// The line item list to which the special pricing being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records the special pricing that has been added, removed, or modified.
			SpecialPricingChange SpecialPricingChangeType
		}

		// Use when both the special pricing and the line item list of the group special pricing changes.
		BothListAndSpecialPricingChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupUndefinitizedIdentifier *struct {
		// Use when the only change for a Undefinitized Identifier group is to the list of line items. The requirements description is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the Undefinitized Identifier. The line item list is unchanged.
		UndefinitizedIdentifierChangeOnly []struct {
			// The line item list to which the Undefinitized Identifier being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records a Undefinitized Identifier that has been added or removed.
			UndefinitizedIdentifierModification XsString
		}

		// Use when both the Undefinitized Identifier and the line item list of the group changes.
		BothListAndUndefinitizedIdentifierChange []struct {
			// Records line items that have been added or removed.
			LineItemListChange LineItemListChangeType
	...
	GroupVariablePricing *struct {
		// Use when the only change for a variable pricing group is to the list of line items. The variable pricing is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the variable pricing. The line item list is unchanged.
		VariablePricingChangeOnly []struct {
			// The line item list to which the variable pricing being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records variable pricing that has been added, removed, or modified.
			ItemVariablePricingModification []VariableRangePricingModificationType
		}

		// Use when both the variable pricing and the line item list of the group variable pricing changes.
		BothListAndVariablePricingChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType

			// Records variable pricing that has been added, removed, or modified.
			ItemVariablePricingChange []VariableRangePricingModificationType
		}
	}

	// Changes to the zone pricing that apply to a group of procurement instrument line items.
	GroupZonePricing *struct {
		// Use when the only change for a zone pricing group is to the list of line items. The zone pricing is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the zone pricing. The line item list is unchanged.
		ZonePricingChangeOnly []struct {
			// The line item list to which the zone pricing being modified is associated.
			LineItemList []LineItemIdentifierType

			// Records the zone pricing that has been added, removed, or modified.
			ZonePricingModification ZonePricingModificationType
		}

		// Use when both the zone pricing and the line item list of the group zone pricing changes.
		BothListAndZonePricingChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
	...
	GroupWageDetermination *struct {
		// Use when the only change for a wage determination group is to the list of line items. The wage determination is unchanged.
		LineItemListChangeOnly []LineItemListChangeType

		// Use when the only change is to the wage determination. The line item list is unchanged.
		WageDeterminationChangeOnly []struct {
			// The line item list to which the zone pricing being modified is associated.
			LineItemList []LineItemIdentifierType
		...
		// Use when both the wage determination and the line item list of the group wage determination changes.
		BothListAndWageDeterminationChange []struct {
			// Records line items that have been added, removed, or modified.
			LineItemListChange LineItemListChangeType
...
type ProcurementInstrumentAmountsModificationType struct {
	...
	UndefinitizedOtherAmountsModificationDetails *struct {
		...
		// Used to record changes to an existing undefinitized other amounts that does not result in a zero undefinitized other amount.
...
type ProcurementInstrumentHeaderModificationDetails struct {
	...
	ModificationDetails struct {
		...
		// The reason for the modification. Required for modifications. (see Modification Reason list at dodprocurementtoolbox.com)
...
type ReplacementPriceModificationType struct {
	...
	DeletedReplacementPrice *ReplacementPriceType

	// Contains the replacement price that is modified.
	ChangedReplacementPrice *struct {
...
type RoutingTransitModificationType struct {
	// When TransportationMethod contains "Pipeline", provide the Department of Defense Activity Address Code (DoDAAC) of the pipeline."
	PipelineDoDAAC *DoDAACType
...
type SpecialPricingChangeType struct {
	// Contains the special pricing that is added.
	AddedSpecialPricing *SpecialPricingType

	// Contains the special pricing that is removed.
	DeletedSpecialPricing *SpecialPricingType

	// Contains the special pricing that is modified.
	ChangedSpecialPricing *struct {
...
type VariableRangePricingModificationType struct {
	// The type of range based pricing being modified. (see list)
	RangeBasedPricingType XsToken

	// Records quantity ranges and prices that have been added, removed, or modified.
	QtyRangeModificationDetails struct {
		// Contains quantity ranges and prices that are added.
		AddedQtyRange []QtyAmountRangeType

		// Contains quantity ranges and prices that are removed.
		DeletedQtyRange []QtyAmountRangeType

		// Contains quantity ranges and prices that are modified.
		ChangedQtyRange []struct {
			// Description of the change to the quantity range and price.
			QtyRangeChangeText XsString

			// The quantity range and price after the changes were applied. The results.
			CurrentQtyRange QtyAmountRangeType

			// The quantity range and price before the changes were applied. The original.
			PreviousQtyRange QtyAmountRangeType
		}
	}
}
...
type ZonePricingModificationType struct {
	// Contains the zone pricing that is added.
	AddedZonePricing *ZonePricingType

	// Contains the zone pricing that is removed.
	DeletedZonePricing *ZonePricingType

	// Contains the zone pricing that is modified.
	ChangedZonePricing *struct {
...
type AccountingDetailType1 struct {
	// Type of financial reference. (see list)
	FinancialCodeDescription XsToken
...
type AttachmentType struct {
	...
	// The format of the attachment such as Microsoft Word, Lotus 1.2.3 etc. Required when Attachment Binary Text is provided. (see Attachment Data Format list at dodprocurementtoolbox.com)
	AttachmentDataFormat *XsString

	// The date of the attachment in CCYY-MM-DD format.
...
type ClauseDetailsType struct {
	// Regulation or Supplement name such as FAR, DFARS and AFARS. (see list)
	RegulationOrSupplement *ClauseRegulationSupplementType

	// Issuing Activity name if clause or provision source is not found in the RegulationOrSupplement list. For example, NAVAIR and MICOM.
	...
	ClauseVariation []struct {
		...
		// When applicable, the alternate or deviation name. For example, for "Alternate A", value in VariationName would be "A".
...
type ContractObligatedAmountType struct {
	...
	FinancialReference []AccountingDetailType1
...
type DavisBaconActType struct {
	...
	// The previous wage determination number that the wage determination provided in GeneralDecisionNumber replaces.
	SupersededGeneralDecisionNumber *XsString

	// List of modifications to the wage determination provided in GeneralDecisionNumber.
...
type DiscountValueType1 struct {
	...
	// When ValueDescription is "Percentage", the base against which the percentage is applied. (see list)
	PercentageBasis *XsToken
}

// Qualifier (without quantity), Value, Percent Basis with specific pick lists for OtherDiscounts
...
type ExhibitLineItemType struct {
	...
	// The exhibit serial number. If the ElinBase is one character, then provide 3 characters: position 1 must be 0-9, positions 2 and 3 must be 0-9 and A-Z. If the ElinBase is two characters, then provide 2 characters (0-9 and A-Z). Do no use the letter I or O.
...
type FoBDetailsFINALType struct {
	...
	// When the FoBPoint contains "Terminal", provide the 5 position United Nations code for ports and other locations (UN/LOCODE) in accordance with the United Nations Location Code Manual to identify the FoB Point.
...
type ItemObligatedAmountType struct {
	...
	// One or more codes or numbers referencing a funding line. Accounting Classification Reference Number (ACRN) must be provided.
	FinancialReferenceCode []AccountingDetailType1
}
...
type LineItemBasicInformationType struct {
	...
	// Denotes whether or not the line item is an option line item.
...
// Quantity units for the price. 3 for $10
type QuantityPerPriceType struct {
	// Price per unit.
	Price XsFloat
...
type ReplacementPriceType QuantityPerPriceType
...
// Step ladder and incremental pricing for solicitations.
type SolicitatedVariableRangePricingType struct {
	// The type of range based pricing. (see list)
	RangeBasedPricingType XsToken

	// The range and the price for that range.
	QtyRange []QtyAmountRangeType
...
// Information specific to telecomm contracts.
...
type UnitPriceType1 struct {
	...
	// Range based pricing details.
	VariableRangePricing []VariableRangePricingType
...
type VariableRangePricingType struct {
	// The type of range based pricing. (see list)
	RangeBasedPricingType XsToken
